...
class Rental(models.Model):
    ...
    car = models.ForeignKey(
        "fleet.car", on_delete=models.PROTECT, related_name="rentals"
    )
    ...
    def clean(self):
        ...
        if self.end_date > max_allowed_date:
            raise ValidationError(
                {"end_date": "Hire is allowed for only 5 years, not more"}
            )

        # Overlapping
        check_car_availability(
            self.car, self.start_date, self.end_date, current_rental_id=self.id
        )
